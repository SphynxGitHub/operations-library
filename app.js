...
import * as OLOnboarding from './features/onboarding.js';
import * as OLNotifications from './features/notifications.js';
import * as OLRoles from './features/roles.js';
...
window.addEventListener("load", async () => {
    ...
    if (state.currentUser && !window.IS_GUEST && typeof OL.startNotificationPolling === 'function') {
        OL.startNotificationPolling();
    }
});
...
// ---------------------------------------------------------------
// SHARED SEARCHABLE PROJECT PICKER — a type-to-filter client/project
// dropdown, factored out of the Quick Task Creator's version so it can be
// reused anywhere a plain <select> of every client is unwieldy (currently:
// Communications' project filter, Error Tracking's client filter/assign).
// Config per instance is kept on OL._searchablePickers[idPrefix] since the
// inline onmousedown/onfocus handlers can only pass strings, not closures.
// ---------------------------------------------------------------
OL._searchablePickers = {};

// idPrefix: unique string per instance (becomes element ids).
// clients: array of client objects to offer.
// selectedId/selectedLabel: current value, if any (pre-fills the input).
// allOptionLabel: label for the "clear" option (e.g. "All Projects"); pass
//   null to omit it (e.g. for an assign-to picker with no "all" concept).
// onSelect: name of a global OL.* function to call with (clientId, label)
//   when an option is picked.
OL.renderSearchableProjectPicker = function({ idPrefix, clients, selectedId = "", selectedLabel = "", allOptionLabel = "All Projects", extraOptions = [], onSelect, placeholder = "Search projects..." }) {
    OL._searchablePickers[idPrefix] = { clients, allOptionLabel, extraOptions, onSelect };
    return `
        <div class="qtf-field" style="position:relative; min-width:200px;">
            <input type="hidden" id="${idPrefix}-value" value="${esc(selectedId)}">
            <div style="position:relative; display:flex; align-items:center;">
                <i data-lucide="search" style="position:absolute; left:8px; width:12px; height:12px; color:var(--muted); pointer-events:none;"></i>
                <input type="text" id="${idPrefix}-search" class="modal-input tiny"
                       style="padding-left:24px; width:100%; box-sizing:border-box;"
                       placeholder="${esc(placeholder)}" autocomplete="off"
                       value="${esc(selectedLabel)}"
                       onfocus="OL.populateSearchableProjectPicker('${idPrefix}', this.value)"
                       oninput="OL.populateSearchableProjectPicker('${idPrefix}', this.value)">
            </div>
            <div id="${idPrefix}-results"
                 style="display:none; position:absolute; top:100%; left:0; right:0; z-index:9999; max-height:220px; overflow-y:auto; background:var(--panel-dark, #0f172a); border:1px solid var(--line); border-radius:6px; margin-top:4px; box-shadow:0 10px 25px rgba(0,0,0,0.5); padding:4px;">
            </div>
        </div>
    `;
};

OL.populateSearchableProjectPicker = function(idPrefix, query) {
    const resultsContainer = document.getElementById(`${idPrefix}-results`);
    const config = OL._searchablePickers[idPrefix];
    if (!resultsContainer || !config) return;

    const cleanQuery = (query || "").toLowerCase().trim();
    const matches = cleanQuery
        ? config.clients.filter(c => (c.meta?.name || "").toLowerCase().includes(cleanQuery))
        : config.clients;

    let html = config.allOptionLabel ? `
        <div class="tiny" style="padding:6px 10px; border-radius:4px; cursor:pointer; font-weight:600; color:var(--accent);"
             onmouseover="this.style.background='rgba(56,189,248,0.12)'" onmouseout="this.style.background='transparent'"
             onmousedown="OL.selectSearchableProjectPicker('${idPrefix}', '', '${esc(config.allOptionLabel).replace(/'/g, "\\'")}')">
            ${esc(config.allOptionLabel)}
        </div>
    ` : "";

    html += (config.extraOptions || []).map(opt => `
        <div class="tiny" style="padding:6px 10px; border-radius:4px; cursor:pointer;"
             onmouseover="this.style.background='rgba(56,189,248,0.12)'" onmouseout="this.style.background='transparent'"
             onmousedown="OL.selectSearchableProjectPicker('${idPrefix}', '${opt.value}', '${esc(opt.label).replace(/'/g, "\\'")}')">
            ${esc(opt.label)}
        </div>
    `).join('');
    ...
    html += matches.length
        ? matches.map(c => `
            <div class="tiny" style="padding:6px 10px; border-radius:4px; cursor:pointer; display:flex; align-items:center; gap:6px;"
                 onmouseover="this.style.background='rgba(56,189,248,0.12)'" onmouseout="this.style.background='transparent'"
                 onmousedown="OL.selectSearchableProjectPicker('${idPrefix}', '${c.id}', '${esc(c.meta?.name || c.id).replace(/'/g, "\\'")}')">
                <i data-lucide="folder" style="width:12px;height:12px;color:#38bdf8;"></i> ${esc(c.meta?.name || c.id)}
            </div>
        `).join("")
        : (cleanQuery ? `<div class="tiny muted" style="padding:6px 10px;">No matching projects</div>` : "");
...
OL.selectSearchableProjectPicker = function(idPrefix, clientId, label) {
    const config = OL._searchablePickers[idPrefix];
    ...
    if (searchInput) searchInput.value = clientId ? label : "";
    if (resultsContainer) resultsContainer.style.display = "none";

    if (config?.onSelect && typeof OL[config.onSelect] === "function") {
        OL[config.onSelect](clientId, label);
    }
};

document.addEventListener("click", function(e) {
    Object.keys(OL._searchablePickers || {}).forEach(idPrefix => {
        const resultsContainer = document.getElementById(`${idPrefix}-results`);
        const searchInput = document.getElementById(`${idPrefix}-search`);
...
OL.renderSystemUtilityBar = function() {
    return `
        <div class="system-utility-bar" style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
            <button class="btn tiny soft" onclick="OL.toggleTheme()" title="Toggle Light/Dark Theme" style="display:inline-flex; align-items:center; gap:4px;">
                <i data-lucide="sun-moon" style="width:13px;height:13px;"></i> Theme
            </button>
            ${isStaffSession() ? `
...
window.buildLayout = function () {
    ...
    const masterTabs = [
        { key: "vault-apps", label: "Master Apps", icon: "layout-grid", href: "#/vault/apps" },
        { key: "vault-functions", label: "Master Functions", icon: "wrench", href: "#/vault/functions" },
        { key: "vault-resources", label: "Master Resources", icon: "database", href: "#/vault/resources" },
        { key: "vault-visualizer", label: "Flow Map", icon: "workflow", href: "#/vault/visualizer" },
        { key: "vault-how-to", label: "Master How-To Guides", icon: "book-open", href: "#/vault/how-to" },
        { key: "vault-tasks", label: "Master Tasks", icon: "clipboard-list", href: "#/vault/tasks" },
        { key: "vault-analyses", label: "Master Analyses", icon: "trending-up", href: "#/vault/analyses" },
        { key: "vault-rates", label: "Scoping Rates", icon: "circle-dollar-sign", href: "#/vault/rates" },
        { key: "vault-data", label: "Master Data Tags", icon: "tag", href: "#/vault/data" },
        { key: "vault-automations", label: "Automation Rules", icon: "zap", href: "#/vault/automations" },
        { key: "vault-client-access", label: "Client Logins", icon: "key-round", href: "#/vault/client-access" }
    ];
   ...
   //================ SIDEBAR CONTENT HTML =================//
    const permittedBusinessTabs = businessTabs.filter(item => 
        (isAdmin || effectiveAdminMode) || (typeof OL.hasTeamPermission === 'function' && OL.hasTeamPermission(item.key))
    );

    const permittedMasterTabs = masterTabs.filter(item => 
        (isAdmin || effectiveAdminMode) || (typeof OL.hasTeamPermission === 'function' && OL.hasTeamPermission(item.key))
    );
    ...
    const sidebarContent = `
        ...
        <div class="sidebar-inner-content" style="${isSidebarCollapsed ? 'display:none;' : 'display:flex; flex-direction:column; justify-content:space-between; height:100%;'}">
            <div class="sidebar-padding" style="padding: 10px; flex:1;">
                ...
                ${showHome ? `
                <div class="admin-nav-zone" style="display:flex; align-items:center; gap:8px;">
                    <nav class="menu" style="flex:1; padding:0; margin:0;">
                    ...
                    <span id="ol-notification-bell-slot">${typeof OL.renderNotificationBell === 'function' ? OL.renderNotificationBell() : ''}</span>
                </div>
                ...
                ${(isAdmin || effectiveAdminMode || state.teamMemberMode) && !state.activeClientId ? `
                    ...
                    <div class="client-nav-zone admin-workspace">
                        ...
                        <nav class="menu">
                            ${permittedBusinessTabs.map(item => `
...
window.handleRoute = function () {
    ...
    // If ?client=xyz is in the URL but that client only exists locally as a
    // lightweight stub (e.g. from the client list) rather than the fully
    // loaded record, render against it anyway and you get an empty
    // resources/tasks list, plus newly-created items that vanish — the
    // stub's projectData doesn't reflect what's actually on the server.
    // This happens for anyone who lands on a client's workspace URL
    // without going through switchClient() first (a bookmark, a refresh,
    // a shared link) — switchClient() itself already loads fully, so this
    // only fires for the paths that skip it.
    const clientIdParam = new URLSearchParams(window.location.search).get('client');
    if (clientIdParam && client && (client._metaOnly || !client.projectData)) {
        if (main) {
            main.innerHTML = `
        ...
        OL.loadFullClient(clientIdParam).then(() => window.handleRoute());
        return;
    }
    ...
    if (hash.startsWith('#/business')) {
        ...
        // Team members only get in if the roster grants that specific tab
        // (admins always pass). Route guard mirrors the nav filter above —
        // without this, an unpermitted person could still reach a page by
        // typing/pasting its hash directly, even with no nav link shown.
        const isFullAdmin = window.FORCE_ADMIN === true || (!new URLSearchParams(window.location.search).has('access') && state.adminMode);
        const routeKeyMatch = hash.match(/^#\/business\/([a-z-]+)/);
        const routeKey = routeKeyMatch ? routeKeyMatch[1] : '';
        if (!isFullAdmin && state.teamMemberMode && routeKey && routeKey !== 'dashboard' &&
            typeof OL.hasTeamPermission === 'function' && !OL.hasTeamPermission(routeKey)) {
            const main = document.getElementById('mainContent');
            if (main) main.innerHTML = `<div class="p-20 muted text-center">You don't have access to this section. Ask an admin to grant it from the Sphynx Team page.</div>`;
            return;
        }
    ...
    if (isVault) {
        ...
        // Extract the path key (e.g., "#/vault/apps" -> "vault-apps")
        const routeKeyMatch = hash.match(/^#\/vault\/([a-z-]+)/);
        const vaultKey = routeKeyMatch ? `vault-${routeKeyMatch[1]}` : '';

        const isFullAdmin = window.FORCE_ADMIN === true || (!new URLSearchParams(window.location.search).has('access') && state.adminMode);

        // 🛡️ SECURITY GUARD: Block guests and team members who lack permission for this specific Vault tab
        const hasAccess = isFullAdmin || (state.teamMemberMode && typeof OL.hasTeamPermission === 'function' && OL.hasTeamPermission(vaultKey));
            ...
            if (main) {
                main.innerHTML = `
                    <div style="padding:40px; text-align:center; color:var(--muted);">
                        <h3>Access Denied</h3>
                        <p style="margin-top:8px;">You do not have permission to view this Template Vault section.</p>
                        <p class="tiny" style="margin-top:4px;">Ask a Master Admin to grant access from the Sphynx Team page.</p>
                    </div>`;
            }
            return;
        }

        // Render Permitted Route
...
OL.handleGlobalSearch = function(query) {
    ...
    // (e.g. mid-creation, or an older record from before a field existed).
    // Calling .toLowerCase() straight on it threw on the very first bad
    // record and aborted the whole filter — silently breaking search for
    // every client, not just the one missing a name.
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Enter') {
        ...
        // Exclude inline link inputs
        if (e.target.id && e.target.id.startsWith('inline-link-input-')) {
            return;
        }

        // Exlude textboxes and comment box
        if (e.target.tagName === 'TEXTAREA' || e.target.isContentEditable || e.target.getAttribute('contenteditable') === 'true') {
