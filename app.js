...
OL.persist = async function() {
    ...
    window.saveTimeout = setTimeout(async () => {
        ...
        try {
            ...
            // 🚀 MAP CAMELCASE TO SNAKE_CASE SUPABASE COLUMNS
            const masterPayload = {
                id: 'main_state',
                exported_at: new Date().toISOString(), // 👈 FIX: Satisfies NOT NULL constraint
...
window.renderClientDashboard = function() {
    ...
    if (isPartnerRoute || window.IS_GUEST || accessToken) {
        if (activeClient) {
            // Find the Partner ID (either the project itself if status is 'Partner', or its partnerOwner link)
            const partnerId = activeClient.meta?.status === "Partner" 
                ? activeClient.id 
                : activeClient.meta?.partnerOwner;

            if (partnerId) {
                // Partner access / Portfolio view: Show ONLY the sub-projects assigned to this partner + the partner project itself
                clients = clients.filter(c => String(c.id) === String(partnerId) || String(c.meta?.partnerOwner) === String(partnerId));
            } else {
                // Direct client access: show only this specific client project
...
OL.universalPrint = function() {
    ...
    textareas.forEach((ta) => {
        ...
        div.setAttribute('style', 'white-space: pre-wrap; width: 100%; display: block; color: black; padding: 5px 0; fo