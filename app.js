...
window.renderClientDashboard = function() {
    ...
    // 🛡️ SECURITY GUARD: Filter projects if accessing via token/guest mode
    const urlParams = new URLSearchParams(window.location.search);
    const accessToken = urlParams.get('access');
    const activeClient = getActiveClient();

    if (window.IS_GUEST || accessToken) {
        if (activeClient) {
            const partnerId = activeClient.meta?.status === "Partner" ? activeClient.id : activeClient.meta?.partnerOwner;
            if (partnerId) {
                // Partner access: show only the partner project + its linked sub-clients
