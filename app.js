...
OL.initWBMotion = function(e, id) {
    ...
    const onUp = async (uE) => {
        ...
        await OL.updateAndSync(() => {
            ...
            // 🚀 1. FIND CLOSEST NEIGHBOR (Any Lane)
            // Look for any card within 150px vertically and 300px horizontally
            const neighbors = allResources.filter(r => 
                r.id !== res.id && 
                r.coords && 
                !r.isGlobal &&
                Math.abs(r.coords.y - dropY) < 100 && // Same "Row"
                Math.abs(r.coords.x - dropX) < 300    // Proximity
            );

            if (neighbors.length > 0) {
                // Sort to find the one physically closest to the drop point
                neighbors.sort((a, b) => {
                    const distA = Math.hypot(a.coords.x - dropX, a.coords.y - dropY);
                    const distB = Math.hypot(b.coords.x - dropX, b.coords.y - dropY);
                    return distA - distB;
                });

                const closest = neighbors[0];
                // Snap to the right of the closest card
                res.coords = { 
                    x: closest.coords.x + 240, 
                    y: closest.coords.y 
                };
                console.log(`🧲 Snapped to right of: ${closest.name}`);
            } else {
                // No neighbors? Just drop where the mouse is
                res.coords = { x: dropX, y: dropY };
            }

            // 🚀 2. RE-ASSIGN LANE BASED ON FINAL SNAP POSITION
            let accumulatedX = 0;
            for (let s of stages) {
                const w = s.width || 300;
            ...
            // 🚀 3. UPDATE ALL LANE WIDTHS (The "Elastic" expansion)
            // We recalculate every lane to ensure the dashed lines move correctly
            let runningTotalX = 0;
