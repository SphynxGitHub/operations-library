...
OL.initWBMotion = function(e, id) {
    ...
    const onUp = async (uE) => {
        ...
        await OL.updateAndSync(() => {
            ...
            // 1. FIND CLOSEST NEIGHBOR (Global Search)
            const neighbors = allResources.filter(r => 
                String(r.id) !== String(id) && 
                r.coords && 
                !r.isGlobal &&
                Math.abs(r.coords.y - dropY) < 100 && // Row threshold
                Math.abs(r.coords.x - dropX) < 300    // Magnet reach
            );

            if (neighbors.length > 0) {
                // Sort by proximity to drop point
                neighbors.sort((a, b) => {
