...
OL.recalculateLaneWidths = async function() {
    ...
    sourceData.stages.forEach((stage, i) => {
        ...
        const cardsInLane = allResources.filter(r => (r.gridLane === laneId || r.stageId === laneId) && r.coords);
        
        let minRequiredWidth = 300;
        if (cardsInLane.length > 0) {
            // Find furthest right card in THIS specific lane
            const maxRightEdge = Math.max(...cardsInLane.map(r => r.coords.x + 280));
            
            // Calculate lane start position
            const laneStart = sourceData.stages.slice(0, i).reduce((sum, s) => sum + (s.width || 300), 0);
            
            // Required width is how far the card is from the lane's left edge
            minRequiredWidth = Math.max(300, (maxRightEdge - laneStart) + 50); 
        }

        if (stage.width !== minRequiredWidth) {
            stage.width = minRequiredWidth;
            needsUpdate = true;
        }
    });

    if (needsUpdate) {
        await OL.persist(); // 🚀 Crucial: Save the new widths to the database
