...
import { deriveWorkStatus, WORK_STATUS_LABELS, ASK_KINDS } from '../core/work-status.js';
...
export function renderScopingRow(item, idx, showUnits) {
    ...
    let workHtml = '';
    if (isActiveRow) {
        const w = deriveWorkStatus(item, client?.projectData?.clientTasks || [], { closedNames: closedStatusNames() });
        const waiting = w.status !== 'pending_sphynx_action';
        const color = waiting ? '#f59e0b' : '#64c6a2';
        workHtml = `
