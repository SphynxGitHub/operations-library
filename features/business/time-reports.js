...
OL.renderBusinessTimeReports = function() {
    ...
    main.innerHTML = `
        ...
        <div class="card" style="padding: 20px;">
            <div style="display: flex; gap: 12px; align-items: center; margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid var(--line);">
                <div style="display: flex; gap: 8px; flex: 1; align-items:center;">
                    <i data-lucide="search" style="width:16px;height:16px;color:var(--muted);"></i>
                    <input type="text" class="modal-input tiny" placeholder="Filter entries..." value="${esc(OL.globalTaskFilterState?.query || '')}" oninput="OL.setGlobalTaskFilter('query', this.value); OL.renderBusinessTimeReports();">
                </div>
            </div>
            <div>${OL.renderTimeReportTableGroups(masterTasks, hourlyRate)}</div>
        </div>
    `;
...
OL.renderTimeReportTableGroups = function(allTasks, hourlyRate) {
    ...
    let filtered = allTasks.filter(t => (t.title || t.name || '').toLowerCase().includes(query) || (t.clientName || '').toLowerCase().includes(query));

    if (filtered.length === 0) return `<div class="p-20 muted text-center">No time entries found.</div>`;
            ...
            <tbody>
                ${filtered.map(t => {
                    ...
                    return `
                        <tr>
                            <td><strong>${esc(t.title || t.name)}</strong></td>
                            <td style="text-align:center;"><span class="pill tiny soft" style="cursor:pointer;" onclick="OL.navigateToClientProject('${t.clientId}')">📁 ${esc(t.clientName)}</span></td>
...
OL.openTimeReportModal = function(selectedClientId) {
    ...
    const content = `
        <div style="padding: 20px; max-width: 900px; width: 100%;" onclick="event.stopPropagation()">
            ...
            <div id="reconciliation-report-content">${OL.renderClientReportView(targetClientId)}</div>
        </div>
    `;
    ...
    if (typeof window.openModal === 'function') {
        ...
        requestAnimationFrame(() => {
            ...
            if (layer) layer.onclick = (e) => { if (e.target === layer) OL.closeTimeReportModal(); };
        });
        if (window.lucide) lucide.createIcons();
    }
};
