...
const SHEET_STATUS_FIELDS = [
    { key: 'sheetStatus', label: 'New Scoping Sheet Status' },
    { key: 'previousSheetStatus', label: 'Previous Scoping Sheet Status' }
];

const REQUEST_ACTIVATED_FIELDS = [
...
OL.getAutomationConditionFields = function(trigger) {
    if (trigger === 'scoping_request_activated') return REQUEST_ACTIVATED_FIELDS;
    if (trigger === 'scoping_sheet_status_change') return SHEET_STATUS_FIELDS;
...
OL.getAutomationConditionValueOptions = function(field) {
    if (field === 'requestType') return getRequestTypes().map(t => t.key);
    if (['sheetStatus', 'previousSheetStatus'].includes(field)) return SHEET_STATUSES;
...
OL.executeAutomationAction = function(rule, ctx) {
    ...
    if (action.type === 'apply_blueprint') {
        ...
        const newTask = OL.buildTaskFromBlueprint(blueprint, client, {
            ...
            requestLineItemId: ctx.requestLineItemId
        });
    ...
    const fill = (str) => String(str || '')
        ...
        .replace(/\{sheetStatus\}/g, ctx.sheetStatus || '')
        .replace(/\{requestTitle\}/g, ctx.requestTitle || '');
...
OL.installStarterRequestSops = function() {
    ...
    updateAndSync(() => {
        ...
        STARTER_REQUEST_SOPS.forEach(def => {
            const items = def.steps.map(step => {
                const title = `${step}: {resourceName}`;
                let bp = state.master.taskBlueprints.find(b => b.title === title);
                if (!bp) {
                    bp = { id: uid(), title, description: '', defaultAssignee: 'Sphynx Task',
                           defaultStatus: 'Pending Sphynx Action', dueInDays: null, howToIds: [], createdAt: now };
                    state.master.taskBlueprints.push(bp);
                    summary.templates++;
                }
                return { blueprintId: bp.id, dueMode: 'default' };
            });

            const sopName = `Request: ${def.label}`;
            let sop = state.master.sops.find(x => x.name === sopName);
            if (!sop) {
                sop = { id: uid(), name: sopName, description: `Starter steps for a ${def.label.toLowerCase()} request`,
                        items, blueprintIds: items.map(i => i.blueprintId), createdAt: now };
        ...
        <div class="modal-body">
            ...
            <div class="card-section">
                ...
                <div id="auto-action-freeform" style="${action.type === 'apply_blueprint' ? 'display:none;' : ''}">
                    <label class="tiny muted uppercase bold">Title (use {resourceName}, {taskTitle}, {clientName}, {eventTitle}, {sheetStatus}, {requestTitle})</label>
