...
export function renderHowToLibrary() {
    ...
    container.innerHTML = `
        ...
        ${visibleGuides.length === 0 ? '<div class="empty-hint" style="text-align: center; padding: 60px; opacity: 0.5;">No guides found in this library.</div>' : renderHowToGroupedByCategory(visibleGuides, client, isVaultView)}
    `;
...
// Groups the how-to grid/list by ht.category (defaulting to "General" for
// anything uncategorized), sorted alphabetically with "General" last so the
// named categories lead.
function renderHowToGroupedByCategory(guides, client, isVaultView) {
    const grouped = guides.reduce((acc, ht) => {
        const cat = ht.category || 'General';
        if (!acc[cat]) acc[cat] = [];
        acc[cat].push(ht);
        return acc;
    }, {});
    ...
    const isList = OL.getViewMode('howto') === 'list';

    return `
        <div class="howto-categories-wrapper" style="margin-top:20px;">
            ${sortedCats.map(cat => `
                <div class="resource-group" style="margin-bottom: 32px;">
                    <div style="display:flex; align-items:center; gap:8px; border-bottom: 1px solid var(--accent); padding-bottom: 8px; margin-bottom:15px;">
                        <i data-lucide="folder" style="width:16px; height:16px; color: var(--accent);"></i>
                        <h3 style="margin:0; font-size: 13px; text-transform: uppercase; color: var(--accent); letter-spacing: 0.1em;">${esc(cat)}</h3>
                        <span class="pill tiny soft" style="font-size:9px;">${grouped[cat].length}</span>
                    </div>
                    ${isList ? `
                    ...
                    ` : `
                        <div class="cards-grid">
                            ${grouped[cat].map(ht => renderHowToCard(client?.id, ht, !isVaultView)).join('')}
                        </div>
                    `}
                </div>
            `).join('')}
