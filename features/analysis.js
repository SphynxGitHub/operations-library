            ...
            <div class="card matrix-card-main" style="border-top: 3px solid var(--accent); padding: 20px; margin-bottom: 40px;">
                <div class="section-header">
                    ...
                    <div class="header-actions" style="display:flex; align-items:center; gap:8px;">
                        ...
                        <button class="btn tiny primary" onclick="OL.printAnalysisMatrix('${analysisId}', ${isMaster})" style="display:flex; align-items:center; gap:4px;">
...
// ================= 🖨️ ANALYSIS PDF (styled to match printScopingSheet) =================
export function printAnalysisMatrix(analysisId, isMaster) {
    const client = getActiveClient();
    const source = isMaster ? state.master.analyses : (client?.projectData?.localAnalyses || []);
    const anly = source.find(a => a.id === analysisId);
    if (!anly) return;
    ...
    const allApps = [...(state.master.apps || []), ...(client?.projectData?.localApps || [])];
    const apps = (anly.apps || []).map(appObj => ({
        ...appObj,
        appName: allApps.find(a => a.id === appObj.appId)?.name || appObj.appId || 'Unnamed'
    }));

    const features = [...(anly.features || [])].sort((a, b) => {
        const weightA = OL.getCategorySortWeight(a.category);
        const weightB = OL.getCategorySortWeight(b.category);
        if (weightA !== weightB) return weightA - weightB;
        return (a.category || '').localeCompare(b.category || '');
    ...
    const colTemplate = `220px 64px repeat(${apps.length || 1}, minmax(180px, 1fr))`;

    // ── Rate Card block (pricing tiers per app, mirrors the scoping PDF's item-row styling) ──
    const hasTiers = apps.some(a => (a.pricingTiers || []).length);
    const rateCardHtml = hasTiers ? `
        <div class="category-block">
            <div class="category-title">Pricing &amp; Tiers Definition</div>
            <div class="rate-card-grid" style="grid-template-columns: repeat(${apps.length}, 1fr);">
                ${apps.map(a => `
                    <div class="rate-card-col">
                        <div class="rate-card-app">${esc(a.appName)}</div>
                        ${(a.pricingTiers || []).length ? (a.pricingTiers || []).map(t => `
                            <div class="rate-tier-row"><span>${esc(t.name)}</span><span>$${Number(t.price || 0).toLocaleString()}</span></div>
                        `).join('') : '<div class="rate-tier-row muted">No tiers listed</div>'}
                    </div>
                `).join('')}
            </div>
        </div>
    ...
    features.forEach(feat => {
        ...
        if (catName !== currentCategory) {
            currentCategory = catName;
            rowsHtml += `<div class="category-title">${esc(catName)}</div>`;
        }

        rowsHtml += `
            <div class="feature-row" style="grid-template-columns: ${colTemplate};">
                <div class="feature-name">
                    <div class="feature-name-main">${esc(feat.name)}</div>
                    ${feat.description ? `<div class="feature-desc">${esc(feat.description)}</div>` : ''}
                </div>
                <div class="feature-weight">${Number(feat.weight || 0).toFixed(2)}</div>
                ${apps.map(a => {
                    const score = a.scores?.[feat.id] || 0;
                    const note = a.notes?.[feat.id] || '';
                    return `
                        <div class="feature-cell">
                            <div class="feature-note ${note ? '' : 'muted'}">${note ? esc(note) : '—'}</div>
                            <div class="feature-score-row"><span class="fs-label">Score</span><span class="fs-val">${score}</span></div>
                        </div>
                    `;
                }).join('')}
            </div>
...
.category-block { margin-bottom: 20px; break-inside: avoid; }
.category-title { font-size: 10px; font-weight: 800; text-transform: uppercase;
                   letter-spacing: 0.07em; color: #0ea5e9; padding: 6px 10px;
                   background: #f8fafc; border-left: 3px solid #0ea5e9;
                   border-radius: 0 4px 4px 0; margin: 16px 0 4px; }

.rate-card-grid { display: grid; gap: 12px; margin-top: 6px; }
.rate-card-col { border: 1px solid #e2e8f0; border-radius: 6px; padding: 10px 12px; break-inside: avoid; }
.rate-card-app { font-size: 11px; font-weight: 800; margin-bottom: 6px; }
.rate-tier-row { display: flex; justify-content: space-between; font-size: 10px;
                  padding: 3px 0; border-bottom: 1px solid #f1f5f9; }
.rate-tier-row:last-child { border-bottom: none; }
.rate-tier-row.muted { color: #94a3b8; font-style: italic; }

.matrix-columns-header { display: grid; align-items: end; gap: 10px;
                          padding: 6px 10px; border-bottom: 2px solid #0f172a;
                          margin-bottom: 4px; font-size: 9px; font-weight: 800;
                          text-transform: uppercase; letter-spacing: 0.06em; color: #64748b; }

.feature-row { display: grid; gap: 10px; padding: 8px 10px;
               border-bottom: 1px solid #f1f5f9; break-inside: avoid; }
.feature-name-main { font-size: 11px; font-weight: 700; color: #0f172a; line-height: 1.3; }
.feature-desc { font-size: 9px; color: #64748b; margin-top: 2px; line-height: 1.4; font-style: italic; }
.feature-weight { font-size: 11px; font-weight: 700; color: #0ea5e9; text-align: right; }

.feature-cell { font-size: 10.5px; line-height: 1.4; }
.feature-note { color: #334155; }
.feature-note.muted { color: #cbd5e1; }
...
<div class="print-header">
  ...
  <div class="ph-meta">Generated ${date}<br>Scores: 0 (N/A) · 1 (&lt;60%) · 2 (60-80%) · 3 (80%+)</div>
</div>
${rateCardHtml}
<div class="matrix-columns-header" style="grid-template-columns: ${colTemplate};">
...
Object.assign(window.OL, {
    ...
    addFeatureToAnalysis, removeFeatureFromAnalysis, openCategoryManagerModal,
