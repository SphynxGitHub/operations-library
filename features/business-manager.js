// ⚠️ NOT LOADED — app.js imports './features/business/index.js' (the
// features/business/ folder: tasks.js, time-reports.js, calendar.js, etc.),
// not this file. Nothing here runs in the live app; confirmed by grepping
// for "business-manager" across app.js and every .html file — no imports
// or script tags reference it. Left in place rather than deleted since
// that's a bigger call than this task asked for, but edits here are inert.
                    ...
                    <div style="display:grid; grid-template-columns: 2fr 130px 140px 110px 240px; gap: 12px; padding: 10px 14px; background: ${isTimerRunning ? 'rgba(56, 189, 248, 0.08)' : 'rgba(255,255,255,0.02)'}; border: 1px solid ${isTimerRunning ? '#38bdf8' : 'var(--line)'}; border-radius: 6px; align-items:center; cursor:pointer;"
                        ...
                        <div>
                            ...
                            <div class="tiny muted" style="display:flex; gap: 8px; align-items:center; margin-top:4px;">
                                ${OL.renderProjectPill(t.clientId, t.clientName)}
...
OL.navigateToClientProject = function(clientId) {
    // Always close whatever modal happens to be open first -- see the
    // matching comment on the other definition of this function in
    // tasks.js (both exist; whichever loads last wins, so both need the
    // same fix rather than relying on load order).
